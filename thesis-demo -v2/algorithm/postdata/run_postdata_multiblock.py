...
from codec import Stage4GlobalDiagonalRangeCodec, Stage4RangeCodec, Stage4TileRangeCodec  # noqa: E402
...
from postdata_blocks import build_manifest, ensure_manifest_blocks_available, load_json  # noqa: E402


DEFAULT_POSTDATA_META = r"E:\code\thesis\experiments\dat\PostData-for-training.sgy.dat.json"
DEFAULT_OUTPUT_DIR = r"E:\code\thesis\20260420\postdata\outputs_postdata_multiblock"
DEFAULT_MANIFEST = r"E:\code\thesis\20260420\postdata\postdata_blocks_manifest.json"

DEFAULT_VAL_RANGES = [(280, 314)]
DEFAULT_TEST_RANGES = [(315, 349)]
...
class BlockStore:
    ...
    def get(self, block_id: str) -> np.ndarray:
        ...
        dat_path = str(block.get("dat_path") or block.get("source_dat_path"))
        shape = tuple(int(v) for v in block["shape"])
        byte_offset = int(block.get("byte_offset", 0))
        floats = np.memmap(dat_path, dtype=np.float32, mode="r", offset=byte_offset, shape=shape)
...
def split_for_profile(profile: int, split_mode: str, val_ranges: Sequence[Tuple[int, int]], test_ranges: Sequence[Tuple[int, int]]) -> str:
    if split_mode == "full_train":
        return "train"
    if in_any_range(profile, test_ranges):
        return "test"
    if in_any_range(profile, val_ranges):
        return "val"
    return "train"
    ...
    for block in manifest["blocks"]:
        ...
        def flush() -> None:
            ...
            if current_split is None or current_count <= 0:
                return
            profile_start = start + current_start
            profile_end = profile_start + current_count - 1
            segment_id = f"{block_id}_{current_split}_p{profile_start:04d}_{profile_end:04d}"
            segments.append(
                Segment(
        ...
        for offset in range(profiles):
            ...
            label = split_for_profile(profile, split_mode, val_ranges, test_ranges)
        ...
        allocation.append(
            {
                ...
                "split": segment.split,
                "profile_range": [segment.profile_start, segment.profile_end],
...
def benchmark_classifier(model: torch.nn.Module, loader: DataLoader, device: str, total_voxels: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "interpretation": "sampled exponent NLL estimate, not a real range-coded bitstream",
    }
...
def split_summary(segments: Sequence[Segment]) -> Dict[str, Any]:
    ...
    for split in ["train", "val", "test"]:
        ...
        payload[split] = {
            ...
            "profile_ranges": [[segment.profile_start, segment.profile_end] for segment in selected],
...
def train_multiblock(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    ensure_manifest_blocks_available(manifest)
    ...
    torch.save(
        {
            ...
            "config": {
                **asdict(config),
                ...
                "training_source": "postdata_multiblock",
    ...
    json_path = args.save_json or os.path.join(args.output_dir, "postdata_multiblock_train_summary.json")
...
def benchmark_checkpoint(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    json_path = args.save_json or os.path.join(args.output_dir, f"postdata_{args.benchmark_split}_benchmark.json")
    ...
    for block in manifest["blocks"]:
        ...
        segments.append(
            Segment(
                ...
                profiles=shape[0],
                profile_start=int(block["profile_start"]),
                profile_end=int(block["profile_end"]),
                shape=shape,
                dat_path=str(block["dat_path"]),
                byte_offset=int(block.get("byte_offset", 0)),
                block_shape=shape,
...
def roundtrip_segments(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if codec_cls is Stage4GlobalDiagonalRangeCodec:
        codec_kwargs["progress"] = not args.no_progress
        codec_kwargs["progress_interval_diags"] = args.progress_every_diags
    if codec_cls is Stage4TileRangeCodec:
        codec_kwargs["tile_shape"] = config.tile_shape
    codec = codec_cls(**codec_kwargs)
    block_store = BlockStore(manifest)
    bitstream_dir = Path(args.output_dir) / f"postdata_{args.roundtrip_split}_{args.codec_layout}_bitstreams"
    bitstream_dir.mkdir(parents=True, exist_ok=True)
    results = []
    total_bytes = 0
    total_voxels = 0
    ok_all: bool | None = None if args.encode_only else True
    for index, segment in enumerate(segments):
        ...
        if isinstance(codec, Stage4GlobalDiagonalRangeCodec):
            codec.progress_label = f"{index + 1}/{len(segments)} {segment.segment_id}"
        mode_label = "Encode-only" if args.encode_only else "Roundtrip"
        print(f"[Info] {mode_label} {index + 1}/{len(segments)} {segment.segment_id}: shape={tuple(volume.shape)}", flush=True)
        if args.encode_only:
            encode_metrics = codec.encode_exponents(volume, str(bitstream_path))
            result_ok = None
            result_header = None
        else:
            ...
            ok_all = bool(ok_all) and result_ok
            encode_metrics = result["encode"]
        encoded_bytes = int(encode_metrics["total_bytes"])
        ...
        if result_header is not None:
            row["header"] = result_header
        results.append(row)
        if args.encode_only:
            print(f"[OK] {index + 1}/{len(segments)} {segment.segment_id}: encoded_bytes={encoded_bytes} (not decoded)")
        else:
            print(f"[OK] {index + 1}/{len(segments)} {segment.segment_id}: ok={result_ok}")
    summary = {
        "manifest": os.path.abspath(args.manifest),
        "checkpoint_path": os.path.abspath(checkpoint),
        "split_mode": args.split_mode,
    ...
    report_stem = "encode" if args.encode_only else "roundtrip"
    json_path = args.save_json or os.path.join(args.output_dir, f"postdata_{args.roundtrip_split}_{args.codec_layout}_{report_stem}.json")
    save_json(json_path, summary)
    print(f"[OK] Saved {report_stem} summary to", json_path)
    return summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PostData multi-block Stage4 exponent training and range coding.")
    parser.add_argument("--action", choices=["prepare", "train", "roundtrip", "benchmark"], required=True)
    parser.add_argument("--postdata-meta", default=DEFAULT_POSTDATA_META)
    parser.add_argument("--manifest", default=DEFAULT_MANIFEST)
    parser.add_argument("--profiles-per-block", type=int, default=0, help="0 keeps PostData as one regular block.")
    ...
    parser.add_argument("--val-range", action="append", default=[], help="Inclusive profile range like 280-314. Can repeat.")
    parser.add_argument("--test-range", action="append", default=[], help="Inclusive profile range like 315-349. Can repeat.")
    ...
    parser.add_argument("--progress-every-diags", type=int, default=25)
    parser.add_argument("--no-progress", action="store_true")
    parser.add_argument("--encode-only", action="store_true", help="Only encode exponent bitstreams; skip decode and verification.")
...
def main() -> int:
    ...
    if args.action == "prepare":
        manifest = build_manifest(
            source_meta_path=args.postdata_meta,
